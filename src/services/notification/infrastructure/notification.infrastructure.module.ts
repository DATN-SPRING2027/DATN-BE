...
import { NOTIFICATION_PERSISTENCE, NOTIFICATION_QUEUES } from './persistence';
...
export class NotificationInfrastructureModule {
  static register(): DynamicModule {
        ...
        ? [
            {
              ...
              useFactory: (config: ConfigService) =>
                new QueueRuntime(config, NOTIFICATION_QUEUES),
            },
          ]
        : []),
    ];
    ...
    if (enabled) {
      imports.push(
        MongoInfrastructureModule.register(NOTIFICATION_PERSISTENCE),
